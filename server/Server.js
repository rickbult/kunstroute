...
serverApplicatie.get("/api/map-punten", async (verzoek, antwoord, volgende) => {
  try {
    // Geregistreerde gebruikers met coördinaten vormen de kaartpunten.
    ...
    antwoord.json(gebruikers.map((u) => {
      ...
      const openDagen = [u.openZaterdag && 'Zaterdag', u.openZondag && 'Zondag'].filter(Boolean).join(' & ');
      return {
        naamKunstenaar:           volleNaam,
        ...
        openDagenKunstroute2026:  openDagen,
